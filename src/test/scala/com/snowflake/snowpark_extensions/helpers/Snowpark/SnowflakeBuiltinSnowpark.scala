...
import com.snowflake.snowpark_extensions.helpers.SnowflakeBuiltin._
import com.snowflake.snowpark_extensions.implicits.DataFrameExtensions.ExtendedDataFrame
...
object SnowflakeBuiltinSnowpark {
  ...
  val df_array = session.createDataFrame(DataFrameCreator.data_for_array).toDF(DataFrameCreator.data_for_array_column)
  val df_for_json = session.createDataFrame(DataFrameCreator.data_for_json).toDF(DataFrameCreator.data_for_json_column)
  val df_for_json_complex = session.createDataFrame(DataFrameCreator.data_for_json_complex).toDF(DataFrameCreator.data_for_json_complex_column)
  ...
  val df_for_rounding = session.createDataFrame(DataFrameCreator.data_for_rounding).toDF(DataFrameCreator.data_for_rounding_column)
  val df_for_regex = session.createDataFrame(DataFrameCreator.data_for_regex).toDF(DataFrameCreator.data_for_regex_column)
  val df_for_signum = session.createDataFrame(DataFrameCreator.data_for_signum).toDF(DataFrameCreator.data_for_signum_column)
  val df_for_double  = session.createDataFrame(DataFrameCreator.data_for_double).toDF(DataFrameCreator.data_for_double_column)
  val df_for_double2  = session.createDataFrame(DataFrameCreator.data_for_double2).toDF(DataFrameCreator.data_for_double2_column)
  val df_for_base64 = session.createDataFrame(DataFrameCreator.data_for_base64).toDF(DataFrameCreator.data_for_base64_column)
  val dfString = session.createDataFrame(DataFrameCreator.data_for_float).toDF(DataFrameCreator.data_for_float_column)
  val df_conv = session.createDataFrame(DataFrameCreator.data_for_conv).toDF(DataFrameCreator.data_for_conv_column)
  ...
  def test_unbase64() = {
      df_for_base64.withColumn("xxx", base64(col("col1"))).withColumn("yyy", unbase64(col("xxx")))
  }
  ...
  def test_unhex(): DataFrame = {
    test_hex().withColumn("string_val", unhex(col("HEX_VALUE")))
  }
  ...
  def test_cbrt(): DataFrame = {
    df_for_json.select(round(cbrt("col2"), lit(10)))
  }

  def test_from_json(): DataFrame = {
    val json_df = df_for_json.select(
      from_json(col("col4")).as("json")
    )
    json_df.selectExpr(
      "json['id']::integer"
      , "json['age']::integer"
      , "json['relative']['id']::integer"
  ...
  def test_date_sub() = {
    var df_test = df_for_cast.selectExpr(
      ...
      , "to_timestamp(col6, 'MM/DD/YYYY HH24:MI:SS') as date_time"
  ...
  def test_last(): DataFrame = {
    ...
    df_window.withColumn("mycol", last(col("col2")) over (partitionWindow))
  }
  ...
  def test_lagDefault(): DataFrame = {
    val partitionWindow = Window.partitionBy(col("col1")).orderBy(col("col1"))
    df_window.withColumn("mycol", lag(col("col2"),1,lit(0)) over (partitionWindow)).orderBy(col("col1"),col("col2"))
  }
  ...
  def test_leadDefault(): DataFrame = {
    val partitionWindow = Window.partitionBy(col("col1")).orderBy(col("col1"))
    df_window.withColumn("mycol", lead(col("col2"),1,lit(0)) over (partitionWindow)).orderBy(col("col1"),col("col2"))
  }
